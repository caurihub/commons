...
describe("PeerDiscovery", () => {
	...
	describe("Connection tests", () => {
		...
		it("should return peers with nft connection", async () => {
			const peerInstance = await PeerDiscovery.new(new CauriHubConnection("http://127.0.0.1/api"));
	...
	describe("Github", () => {
		it("should return with specified network", async () => {
			nock("https://raw.githubusercontent.com/cauriland/peers/main")
				.get("/mainnet.json")
		...
		it("should fail if 404 response is recieved", async () => {
			nock("https://raw.githubusercontent.com/cauriland/peers/main").get("/mainnet.json").reply(404);
		...
		it("should fail if the seed list is empty", async () => {
			nock("https://raw.githubusercontent.com/cauriland/peers/main").get("/mainnet.json").reply(200, []);
		...
		describe("should find valid peer among the mix of valid and invalid ones", () => {
			...
			const initRandomAndNock = async (first: number, second: number) => {
				...
				nock("http://127.0.0.1")
					.get("/api/peers")
					.reply(200, {
						data: [{}, { plugins: {} }, { plugins: { "@cauriland/core-api": {} } }, ...dummyPeers],
					});
